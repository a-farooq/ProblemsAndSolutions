...
void* writer(void* idx)
{
	...
	//sleep(1);
	bal *= 2;
	cout << "writer "<<(int*)idx << " updates the value "<<bal<<endl;
	sem_post(&wrt);
}

void* reader(void* idx)
{
	//{
	...
	cout << "Reader "<<(int*)idx<<" reads the value "<< bal<<endl;
...
int main ()
{
	...
	// spawn threads
  	for (int i=0; i<READERS_COUNT; ++i) {
  		pthread_create(&readers[i], NULL, reader, &a[i]);
		//readers[i] = thread(reader, i+1);
	}
  	for (int i=0; i<WRITERS_COUNT; ++i) {
  		pthread_create(&writers[i], NULL, writer, &a[i]);
