...
Node* constructUtil(int n, int& i, int*pre, char*preLN)
{
    Node* root = nullptr;
    if(i < n)
    {
        root = new Node(pre[i]);
        if(preLN[i] != 'L') {
            root->left = constructUtil(n, ++i, pre, preLN);
            root->right = constructUtil(n, ++i, pre, preLN);
        }
        else {
            root->left=nullptr;
            root->right=nullptr;
...
struct Node *constructTree(int n, int pre[], char preLN[])
{
    // Code here
    int id=0;
    return constructUtil(n, id, pre, preLN);
}
